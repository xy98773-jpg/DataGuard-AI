...
**Dashboard 首页（职责：使用引导 + 数据集总览）**：顶部窄标题条 + 4 张平台统计卡片（运行总数 / 治理成功率 / 发现问题数 / 数据集数，来自 `GET /api/dashboard/stats`）+ **数据集管理列表**（核心）——每行显示自定义名（大字，上传时可命名）+ 文件名副标题 + 来源类型 / 行数 / **最近治理状态**（成功/失败/待审批色标）/ **问题数**徽章，顶部**搜索框（按名称/文件名）+ 来源筛选 + 分页**（数据集多了也能快速定位），每行操作：`治理 →`（跳工作台，自动选中该数据集）、`问题`（跳问题中心并预选该数据集）；底部「如何使用」折叠。**不再放最近运行明细**（运行细节进工作台看）。

**问题中心（职责：按数据集分组的质量问题）**：两级分组第一层显示**数据集自定义名**（文件名作副标题）；顶部**数据集下拉筛选**（显示自定义名、可搜索）+ 严重度/类型筛选；支持 `GET /api/issues?dataset_id=xxx`（Dashboard「问题」按钮跳转预选）。

**数据集列表接口** `GET /api/dataset/list`：返回 `name`（自定义名）/`filename`/`source_type`/`row_count`/`column_count`/`last_status`（该数据集最近一次治理状态）/`issue_count`（跨运行问题数），支持 `search`/`source_type`/`limit`/`offset` 分页。
...
**治理报告导出（HTML / PDF）**：工作台结果区（终态后）与报告弹窗均有「导出 PDF」「导出 HTML」按钮；后端 `GET /api/report/{run_id}/html|pdf` 聚合一次运行的全部真实数据（问题/计划/执行样本/质量维度/审批记录）生成**自包含 HTML**（内嵌 CSS，离线可打开）或**真 PDF**（系统 Edge 无头打印，零浏览器下载）；报告含摘要卡（问题数/清洗成功率/影响行数/质量评分前→后）、按严重级别与类型的纯 CSS 条形图、执行 before→after 样本对比表；中文文件名走 RFC 5987 `filename*` 编码。
...
**任务并发治理**：同时最多运行 **3** 个治理任务（超出返回 429「已达到最大并发任务数」）；治理工作台工具条「运行中任务」面板（紧挨质量趋势按钮）实时列出 PENDING/RUNNING/WAITING_APPROVAL 任务（状态 + 当前节点 + 3 秒自动刷新），**可一键取消**（二次确认）；**取消任务时级联清理该 run 已产生的全部中间数据**（issues/plan/executions/validations/approvals/trace，仅保留 CANCELLED 记录作审计痕迹）——首页统计与问题列表不再残留脏数据；同时若取消的是当前查看的任务，工作台自动重置为初始界面。接口：`GET /api/workflow/active`、`POST /api/workflow/{run_id}/cancel`。
