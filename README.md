...
**治理报告导出（HTML / PDF）**：工作台结果区（终态后）与报告弹窗均有「导出 PDF」「导出 HTML」按钮；后端 `GET /api/report/{run_id}/html|pdf` 聚合一次运行的全部真实数据（问题/计划/执行样本/质量维度/审批记录）生成**自包含 HTML**（内嵌 CSS，离线可打开）或**真 PDF**（系统 Edge 无头打印，零浏览器下载）；报告含摘要卡（问题数/清洗成功率/影响行数/质量评分前→后）、按严重级别与类型的纯 CSS 条形图、执行 before→after 样本对比表；中文文件名走 RFC 5987 `filename*` 编码。Dashboard 新增**质量趋势**入口：移入**治理工作台**顶部工具条「质量趋势」按钮（避免首页过长），点击打开弹窗查看最近 N 次治理的 SVG 双折线（治理前灰虚线 → 治理后蓝实线 + 末端分数）+ 明细表（数据集/时间/前/后/▲提升），数据来自 `GET /api/dashboard/stats` 的 `quality_trend`。
