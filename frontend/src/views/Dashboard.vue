...
// ---- 质量趋势（SVG 折线）----
const trend = computed(() => stats.value?.quality_trend ?? [])
// 生成 before/after 两条折线的 SVG points（viewBox 800x220，Y 轴 0-100 映射）
...
<template>
  <div class="dashboard-page">
    ...
    <!-- 质量趋势（最近治理历史，SVG 折线 + 明细表） -->
    <el-card shadow="never" class="trend-card">
      <template #header>质量趋势（最近 {{ trend.length }} 次治理）</template>
      <template v-if="trend.length >= 2">
        <svg viewBox="0 0 800 220" class="trend-svg" preserveAspectRatio="xMidYMid meet">
          ...
          <!-- 末端 after 分数标注 -->
          <text v-if="trend.length" :x="750" :y="190 - (trend[trend.length - 1]?.after_score ?? 0) * 1.5 - 6" text-anchor="end" class="score-end">
            {{ trend[trend.length - 1]?.after_score }}
          </text>
          <!-- 底部数据集短名 -->
          <text v-for="(l, i) in trendXLabels" :key="i" :x="l.x" y="210" text-anchor="middle" class="axis-label">{{ l.label }}</text>
        </svg>
        <el-table :data="trend" size="small" max-height="260" class="trend-table">
          <el-table-column label="数据集" prop="dataset_name" show-overflow-tooltip />
          <el-table-column label="治理时间" width="150">
